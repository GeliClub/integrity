...
int main (int argc, char** argv)
{
  ...
  for (int i= 0; i <= parameters.numFrames; ++i)
  {
    ...
    {
      cloud_cylinder_id= "cloud_cylinder_" + patch::to_string( std::distance(cylinders.begin(), it) );
    ...
    // if the views are ran on a different thread, pausing execution will not affect the ui, for now this should work
    do {
